...
const PaginationServices = () => {
  ...
  const nextPage = (actualPage)=>{
    if(actualPage >= service.totalPages) return false
  ...
  useEffect(()=>{
    if(service.totalPages){
      let newArray = []
      for (let i = 1; i <= service.totalPages; i++) {
  ...
  return (
    <>
      {/* {service && service.totalPages && (
        <span class="text-sm text-gray-700 dark:text-gray-400 mb-1	">
          Pagina <span class="font-semibold text-gray-900 dark:text-white">{`${actualPage} de ${service.totalPages}`}</span>
        </span>
      )} */}
      ...
      <nav aria-label="Page navigation example">
        <ul className="inline-flex items-center -space-x-px">
          <li>
            <a className="block cursor-pointer px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
        ...
        {service.totalPages && arrayPages.length? (arrayPages.map((ele, i)=>
            <li>
              <a onClick={()=>changePage(ele)} key={i} className="cursor-pointer px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{ele}</a>
            </li>
        )):(
          <li>
